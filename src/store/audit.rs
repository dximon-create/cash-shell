// cash — Audit Log
//
// Every executed command is written to ~/.cash/audit.db (SQLite).
// This is separate from history.db:
//   history.db  — user-facing, editable, used by up/down arrows
//   audit.db    — security-facing, append-only, feeds the Security Engine
//
// Schema:
//   audit(id, session_id, command, cwd, exit_code, kind, hash, recorded_at)
//
// kind: "builtin" | "resolved" | "system" | "unknown"
// hash: SHA-256 of (session_id + command + cwd + recorded_at)
//
// The audit log is APPEND ONLY. No deletes, no updates.
// Tamper detection (Module 9) watches this file for modifications.
...
pub fn record(
    ...
    let conn = open(store)?;
    conn.execute(
        "INSERT INTO audit (session_id, command, cwd, exit_code, kind, hash, recorded_at)
...
pub fn recent(store: &Store, limit: usize) -> anyhow::Result<Vec<AuditEntry>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, session_id, command, cwd, exit_code, kind, hash, recorded_at
           FROM audit
          ORDER BY id DESC
          LIMIT ?1"
    )?;
...
/// Verify the hash of an entry. Returns false if tampered.
...
/// Verify all entries in the audit log. Returns list of tampered IDs.
pub fn verify_all(store: &Store) -> anyhow::Result<Vec<i64>> {
    let conn = open(store)?;
    let mut stmt = conn.prepare(
        "SELECT id, session_id, command, cwd, exit_code, kind, hash, recorded_at FROM audit"
    )?;
    let rows = stmt.query_map([], row_to_entry)?;
    let tampered = rows
        .filter_map(|r| r.ok())
        .filter(|e| !verify_entry(e))
        .map(|e| e.id)
        .collect();
    Ok(tampered)
}
...
mod tests {
    ...
    fn hash_verification_fails_for_tampered_entry() {
        ...
        entry.command = "rm -rf /".to_string(); // tamper
    ...
    fn verify_all_clean_log() {
        ...
        record(&store, "s1", "pwd", "/", 0, CommandKind::System).unwrap();
