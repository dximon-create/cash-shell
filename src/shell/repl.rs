    ...
    loop {
        ...
        match eval(&pipeline, &shell.cwd, &shell.store, &cfg) {
            EvalResult::Ok => {
                ...
                let _ = audit::record(&shell.store, &shell.session_id, &line, &cwd_str, 0, CommandKind::System);
            }
            EvalResult::ChangeDir(new_cwd) => {
                shell.cwd = new_cwd;
                let _ = history::push(&shell.store, &line, &cwd_str, 0, cfg.history_dedup);
                let _ = audit::record(&shell.store, &shell.session_id, &line, &cwd_str, 0, CommandKind::Builtin);
            }
            EvalResult::Exit(code) => {
                let _ = history::push(&shell.store, &line, &cwd_str, code, cfg.history_dedup);
                let _ = audit::record(&shell.store, &shell.session_id, &line, &cwd_str, code, CommandKind::Builtin);
            ...
            EvalResult::Error(e) => {
                ...
                let _ = audit::record(&shell.store, &shell.session_id, &line, &cwd_str, 1, CommandKind::Unknown);
            }
                    ...
                    match parse(&command) {
                        ParseResult::Pipeline(p) => {
                            match eval(&p, &shell.cwd, &shell.store, &cfg) {
                                EvalResult::Ok => {
                                    let _ = history::push(&shell.store, &line, &cwd_str, 0, cfg.history_dedup);
                                    let _ = audit::record(&shell.store, &shell.session_id, &line, &cwd_str, 0, CommandKind::Resolved);
                                }
