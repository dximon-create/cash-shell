...
pub enum EvalResult {
    ...
    NeedsConfirmation { suggestion: String, command: String, confidence: f64 },
}

pub fn eval(pipeline: &Pipeline, cwd: &std::path::Path, store: &Store, config: &Config) -> EvalResult {
    ...
    match executor::run(pipeline) {
        Ok(status) => {
            if status.code == 127 {
                let suggestion = suggester::suggest(&pipeline.stages[0].name, store);
...
fn run_command_string(command: &str, cwd: &std::path::Path, store: &Store, config: &Config) -> EvalResult {
