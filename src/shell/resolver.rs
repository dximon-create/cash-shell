...
pub fn resolve(input: &str, store: &Store, config: &Config) -> ResolveResult {
    ...
    let pattern_matches: Vec<_> = memories
        .iter()
        .filter(|m| m.name.starts_with(&input_lower) || m.name.contains(&input_lower))
        .collect();

    if pattern_matches.len() == 1 {
        let m = pattern_matches[0];
        let confidence = if m.name.starts_with(&input_lower) {
            0.9
        } else {
            0.8
        };
...
mod tests {
    ...
    fn fuzzy_match() {
        ...
        match resolve("lst fls", &store, &config) {
            ResolveResult::Resolved { .. } | ResolveResult::BelowThreshold { .. } => {
