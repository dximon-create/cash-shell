    ...
    loop {
        ...
        match eval(&pipeline, &shell.cwd, &shell.store, &cfg) {
            ...
            EvalResult::NeedsConfirmation { suggestion, command, confidence } => {
                // Ask the user before running a low-confidence match.
                let pct = (confidence * 100.0) as u32;
                eprint!(
                    "cash: did you mean '{}' → {} ({}% match)? [y/N] ",
                    suggestion, command, pct
                );
                io::stderr().flush().ok();
                ...
                if answer.trim().eq_ignore_ascii_case("y") {
                    // Re-parse and run the resolved command.
                    use super::parser::ParseResult;
                    match super::parser::parse(&command) {
